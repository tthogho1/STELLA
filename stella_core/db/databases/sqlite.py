...
class SQLite(DatabaseInterface, ABC):
    ...
    def __init__(self):
        ...
        self.conn = sqlite3.connect(
            # Resolved against the current working directory, like any relative path a
            # Python program is handed -- it is up to whoever starts the process to be in
            # the directory the path is meant to be relative to (app/run.py chdirs into
            # app/ before this runs, so the default "sqlite.db" still lands next to it).
